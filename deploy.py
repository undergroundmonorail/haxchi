...
class wupclient:
    ...
    def __init__(self, ip='192.168.2.219', port=1337):
    ...
    def up(self, local_filename, filename = None):
        ...
        while True:
            ...
            output_max = max(output_max, len(hex(progress)))
            if len(data) < block_size:
                sys.stdout.write(' '*output_max + '\r')
                break
        ret = self.FSA_CloseFile(fsa_handle, file_handle)
...
if __name__ == '__main__':
    ...
    print('Navigating to Sqeak Squad directory...')
    w.cd('usr/title/00050000/101a5600/content/0010')
    print('Navigated to ' + w.pwd() + '.')
    print('Uploading kirby.zip...')
    w.up('installer/data/kirby.zip', 'rom.zip')
    print('Done!')
